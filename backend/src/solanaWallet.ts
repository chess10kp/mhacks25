...
export function getKeyPair(privateKey: string): Keypair {
  try {
    ...
    const walletKeys = Keypair.fromSecretKey(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          "Invalid private key format. Please use base58, base64, or JSON array format.",
        );
      }
    }
  }
...
export async function sendSolana(fromPriv: string, toPub: string, amt: number) {
  try {
    ...
    console.log(
      "From private key (first 10 chars):",
      fromPriv.substring(0, 10) + "...",
    );
    ...
    console.log(
      "Total required (including fee and rent):",
      totalRequired,
      "lamports",
    );
    ...
    if (senderBalance < totalRequired) {
      ...
      throw new Error(
        `Insufficient balance. Available: ${availableSOL.toFixed(6)} SOL, Required: ${requiredSOL.toFixed(6)} SOL (including transaction fee${!recipientExists ? " and rent exemption" : ""})`,
      );
    }
