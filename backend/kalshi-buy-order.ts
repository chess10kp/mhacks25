...
async function findOpenMarket(): Promise<KalshiMarket> {
  ...
  const response = await fetch('https://api.elections.kalshi.com/trade-api/v2/markets/KXNCAAF-26-TTU');
  ...
  console.log(`   📊 Volume 24h: ${market.volume_24h} | Liquidity: ${market.liquidity}`);
...
async function placeBuyOrder(market: KalshiMarket): Promise<KalshiOrder> {
  ...
  console.log(`🔍 Market data: last_price=${market.last_price}, yes_bid=${market.yes_bid}`);
  ...
  const response = await makeAuthenticatedRequest('POST', '/trade-api/v2/portfolio/orders', orderData);
...
async function cancelOrder(orderId: string): Promise<void> {
  ...
  const response = await makeAuthenticatedRequest('DELETE', `/trade-api/v2/portfolio/orders/${orderId}`);
...
// NEW: Amend order
async function amendOrder(orderId: string, newPrice: number, newCount: number): Promise<KalshiOrder> {
  console.log(`\n✏️ Step 6: Amending order...`);
  const amendData = { yes_price: newPrice, count: newCount };
  const response = await makeAuthenticatedRequest('PUT', `/trade-api/v2/portfolio/orders/${orderId}`, amendData);
...
async function main() {
  try {
    ...
    allOrders.forEach((o, index) => {
      ...
      console.log(`      Created: ${new Date(o.created_time).toLocaleString()}`);
      console.log('');
    });
